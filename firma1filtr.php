<?php
	...
	}else
	{
		...
		else
		{
			...
			if(isset($_SESSION['$idFr1'])){
				$idFirmy=$_SESSION['$idFr1'];
				unset($_SESSION['$idFr1']);
			}else{
				$idFirmy=$_POST['idFrSzuk'];
			};
			...
			if(isset($_POST['btn1'])){
				...
				$sql_z2="SELECT * FROM zastepstwa_historia WHERE id_uzytkownika='$id_uzytkownika' AND data2='$data' AND((zakogo='$idFirmy' AND zakogoPrzedr='firma') OR (kto='$idFirmy' AND ktoPrzedr='firma')) ORDER BY zastepstwa_historia.dataMysql";
			...
			if(isset($_POST['szukajwhistorii']))
				{
						...
						$sqlsearch = "SELECT id FROM zastepstwa_historia WHERE id_uzytkownika='$id_uzytkownika' AND data2='$data' AND((zakogo='$idFirmy' AND zakogoPrzedr='firma') OR (kto='$idFirmy' AND ktoPrzedr='firma'))";
						...
						if($rezultquestion=$polaczenie->query($sqlsearch))
							{
								...
								}else
								{
									...
									<input type='hidden' name='inputIdfirmy1' value='".$idFirmy."'>
		   ...
		   <div class="boxzastepstw">
			   ...
			   <table class='table' id='sortable'>
					<tr>
						...
						<th>
							<?php
							 echo "<form method='post'><input type='hidden' name='idFrSzuk' value='".$idFirmy."'><input type='hidden' name='datahidden' value='".$data."'><input type='hidden' name='monthhidden' value='".$month."'><input type='hidden' name='yearhidden' value='".$year."'><input type='submit' name='btn1' class='button1segr' value='data [dz.mies.]'></form>"; 
							 ?>
						</th>
			...
			<div>
			<?php
				echo "<form action='firma1.php' method='post'>
				<input type='hidden' name='inputIdfirmy1' value='".$idFirmy."'>
