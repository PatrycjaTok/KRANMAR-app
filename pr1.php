<?php
	...
	if(!isset($_SESSION['zalogowany']))	//by uniknąć że ktoś niezalogowany wpisze kod w adresie i przejdzie na tą stronę
	{
		...
		else
		{
			...
			if(isset($_SESSION['$idPr1'])){
				$idPracownika=$_SESSION['$idPr1'];
				unset($_SESSION['$idPr1']);
			}else if(isset($_POST['szukajwhistorii'])){
				$idPracownika=$_POST['idPrSzuk'];
			}else{
				$idPracownika=$_POST['inputIdPr1'];
			};
			...
			$sqlimienaziwsko="SELECT pracownicy.id, pracownicy.imie, pracownicy.nazwisko FROM pracownicy WHERE id_uzytkownika='$id_uzytkownika' AND id='$idPracownika'";
			...
			$imienaziwsko=$rowimienazwisko['nazwisko']." ".$rowimienazwisko['imie'];
				...
				$pracownikString="pracownik";
					...
					if(isset($_POST['btn1'])){
						$sql_z2="SELECT * FROM zastepstwa WHERE ((id_uzytkownika='$id_uzytkownika' AND zakogo='$idPracownika' AND zakogoPrzedr='pracownik') OR (id_uzytkownika='$id_uzytkownika' AND kto='$idPracownika' AND ktoPrzedr='pracownik')) ORDER BY zastepstwa.dataMysql";
					}else{
						$sql_z2="SELECT * FROM zastepstwa WHERE ((id_uzytkownika='$id_uzytkownika' AND zakogo='$idPracownika' AND zakogoPrzedr='pracownik') OR (id_uzytkownika='$id_uzytkownika' AND kto='$idPracownika' AND ktoPrzedr='pracownik')) ORDER BY zastepstwa.dataMysql";
					};
					...
					if(isset($_POST['btn1'])){
						$sql_z3="SELECT * FROM zastepstwa_historia WHERE ((id_uzytkownika='$id_uzytkownika' AND zakogo='$idPracownika' AND zakogoPrzedr='pracownik') OR (id_uzytkownika='$id_uzytkownika' AND kto='$idPracownika' AND ktoPrzedr='pracownik')) ORDER BY zastepstwa_historia.dataMysql";
					}else{
						$sql_z3="SELECT * FROM zastepstwa_historia WHERE ((id_uzytkownika='$id_uzytkownika' AND zakogo='$idPracownika' AND zakogoPrzedr='pracownik') OR (id_uzytkownika='$id_uzytkownika' AND kto='$idPracownika' AND ktoPrzedr='pracownik')) ORDER BY zastepstwa_historia.dataMysql";
					};
		...
		<div class=box1>
		   ...
		   <div class="boxzastepstw">
			   <div class="boxzastepstw2"> 
					   ...
					   <button class='switchBtn switchBtnActive btn1radious' onclick='show1()' id='btnSwitch1'>obecne</button>
					   <button class='switchBtn switchBtnHidden btn2radious' onclick='show2()' id='btnSwitch2'> historia</button>
						</div>
				   
				   <br>
				   <div class='search hidden2 hidden'>

				   <form action='pr1filtr.php' method='post'>
					   WYSZUKIWANIE:
					   <br> miesiąc: <input type='number' name='miesiacSearch'>
					   <br> rok: 
					   <?php
					   echo "<input type='hidden' name='idPrSzuk' value='".$idPracownika."'>";
					   
					   $datadoInputu=date("Y");
					   echo "<input type='number' name='rokSearch' value='$datadoInputu'>"; 
					   ?>
					   <input type='submit' name='szukajwhistorii' value='Szukaj' class='radious5px pointer'><br>
					</form>
				</div> 
				<br>
			   ...
			   <div class='hidden1 divTable'>
					...
					<tr>
						...
						<th>
							<?php
							 echo "<form method='post'><input type='hidden' name='inputIdPr1' value='".$idPracownika."'><input type='submit' name='btn1' class='button1segr' value='data [dz.mies.]'></form>"; 
							 ?>
						</th>
								...
								$nrS=$nr-5;
									...
									<div class='editionbar'>
										...
										<a href='#s$nrS'> opcje </a>
											<ul>
												<li><form action='edytujZastepstwo.php' method='post'> <input type='hidden' value='".$id_zastepstwasql2."' name='inputIdZastepstwa'> <input type='hidden' value='".$nr."' name='inputnr'> <input type='hidden' value=true name='is1'> <input type='hidden' value='".$idPracownika."' name='idPracownika'> <input type ='submit' name='edytujZastepstwo' value='edytuj' class='btnedit pointer'> </form></li>
												<li><button onclick='okienkoDeletePrac($id_zastepstwasql2, $idPracownika)' class='btnedit pointer'> usuń </button></li>
											</ul>
										</div>
									</div>
									</td>
				...
				<div class='hidden2 hidden divTable'>
					...
					<tr>
						...
						{ 	$bnr=0;
							...
							while($brow_z2=$brezult_z3->fetch_assoc())
							{

								$bid_zastepstwasql2=$brow_z2['id'];
								$bdatasql=$brow_z2['dataMysql'];
								$bdatasql2=strftime('%d.%m', strtotime($bdatasql));
								$bcosql2=$brow_z2['co'];
								$bgdziesql2=$brow_z2['gdzie'];
								$bzurawsql2=$brow_z2['zuraw'];
								$biloschsql2=$brow_z2['ilosch'];
								$bkwotasql2=$brow_z2['kwota'];
								$buwagisql2=$brow_z2['uwagi'];

								$bzakogoPrzedr=$brow_z2['zakogoPrzedr'];
								$bzakogo=$brow_z2['zakogo'];
								
								$bktoPrzedr=$brow_z2['ktoPrzedr'];
								$bkto=$brow_z2['kto'];
								...
								}else if(($bktoPrzedr)=="innypr")
								{
									...
									@$bktosql=$bktosql8nazwisko." ".$bktosql8imie;
		   ...
		   <div>  
			...
			<div>
			<a href="pracownicy.php"><button class='button2 radious5px pointer'> powrót </button></a>
			</div>
	   </div>
