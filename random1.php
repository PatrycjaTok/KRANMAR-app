    ...
    <div class="container">		
		...
		<div class=box1>
		   ...
		   <div class="boxzastepstw">
			   <div class="boxzastepstw2"> 
					   ...
					   <div class='boxBtnSwitch'>
						...
						<div class='search hidden2 hidden'>
							...
							<form action='random1filtr.php' method='post'>
								WYSZUKIWANIE:
								<br> miesiąc: <input type='number' name='miesiacSearch'>
								<br> rok: 
								<?php
								echo "<input type='hidden' name='idRpSzuk' value='".$idRandom."'>";
				...
				else
				{
					...
					if(isset($_POST['btn1'])){
						$sql_z3="SELECT * FROM zastepstwa_historia WHERE ((id_uzytkownika='$id_uzytkownika' AND zakogo='$idRandom' AND zakogoPrzedr='innypr') OR (id_uzytkownika='$id_uzytkownika' AND kto='$idRandom' AND ktoPrzedr='innypr')) ORDER BY zastepstwa_historia.dataMysql";
						$_SESSION['$idRP1']=$idRandom;
						unset($_POST['btn1']);
					}else{
						$sql_z3="SELECT * FROM zastepstwa_historia WHERE ((id_uzytkownika='$id_uzytkownika' AND zakogo='$idRandom' AND zakogoPrzedr='innypr') OR (id_uzytkownika='$id_uzytkownika' AND kto='$idRandom' AND ktoPrzedr='innypr')) ORDER BY zastepstwa_historia.dataMysql";
			   ...
			   <div class='hidden1 divTable'>
					...
					<tr>
						...
						<th> 
							<?php
							echo "<form method='post'><input type='hidden' name='inputIdPr1' value='".$idRandom."'><input type='submit' name='btn1' class='button1segr' value='data [dz.mies.]'></form> ";
							?>
						</th>
								...
								$nrS=$nr-5;
									...
									<div class='editionbar'>
										...
										<a href='#s$nrS'> opcje </a>
											<ul>
												<li><form action='edytujZastepstwoPr1.php' method='post'> <input type='hidden' value='".$id_zastepstwasql2."' name='inputIdZastepstwa'> <input type='hidden' value='".$nr."' name='inputnr'><input type='hidden' value='".$idRandom."' name='inputIdRP1'> <input type ='submit' name='edytujZastepstwo' value='edytuj' class='btnedit pointer'> </form></li>
												<li><button onclick='okienkoDeleteRand($id_zastepstwasql2, $idRandom)' class='btnedit pointer'> usuń </button></li>
											</ul>
										</div>
									</div>
									</td>
				...
				<div class='hidden2 hidden divTable'>
					...
					<?php
						...
						{ 	$bnr=0;
							...
							while($row_z3=$rezult_z3->fetch_assoc())
							{

								$bid_zastepstwasql2=$row_z3['id'];
								$bdatasql=$row_z3['dataMysql'];
								$bdatasql2=strftime('%d.%m', strtotime($bdatasql));
								$bcosql2=$row_z3['co'];
								$bgdziesql2=$row_z3['gdzie'];
								$bzurawsql2=$row_z3['zuraw'];
								$biloschsql2=$row_z3['ilosch'];
								$bkwotasql2=$row_z3['kwota'];
								$buwagisql2=$row_z3['uwagi'];

								$bzakogoPrzedr=$row_z3['zakogoPrzedr'];
								$bzakogo=$row_z3['zakogo'];
								
								$bktoPrzedr=$row_z3['ktoPrzedr'];
								$bkto=$row_z3['kto'];
							
								if(($bzakogoPrzedr)=="pracownik")
								{
									$bsql_z3a="SELECT pracownicy.id, pracownicy.imie, pracownicy.nazwisko FROM pracownicy WHERE pracownicy.id='$bzakogo'";
									$brezult_z3a=$polaczenie->query($bsql_z3a);
									$brow_z3a=$brezult_z3a->fetch_assoc();

									$bzakogosql3imie=$brow_z3a['imie'];
									$bzakogosql3nazwisko=$brow_z3a['nazwisko'];
		   ...
		   <div>  
			...
			<div>
			<a href="randomPeop.php"><button class='button2 radious5px pointer'> powrót </button></a>
			</div>
	   </div>
