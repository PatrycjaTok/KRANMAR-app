					...
					else
					{
						...
						if(isset($_POST['del'])){
							...
							if($rezultDel=$polaczenie->query($sqlDel))						
							{
								...
								$_SESSION['$idPr1']=$id_pr;
								echo "<script>
									window.close();
									if (window.opener && !window.opener.closed) {
										window.opener.location.href = 'pr1.php';
									};
									</script> ";
									exit();
	...
	<body>
		...
		<?php
			if(@$zakogoAnswer==Null && @$ktoAnswer==Null){
				...
				echo " ".$coAnswer."</b>, <br> dot.: <b>".$zakogoAnswer;
			}else if(@$zakogoAnswer==Null && @$ktoAnswer!==Null){
				echo " ".$coAnswer.", </b><br>dot.: <b>".$ktoAnswer;				
			}else if(@$zakogoAnswer!==Null && @$ktoAnswer!==Null){
				echo "<br> ".$coAnswer."</b>, dot.:<b> ".$zakogoAnswer."</b> oraz <b>".$ktoAnswer;		
			};
				echo "<br> <input type='hidden' name='idZastepstwa' value='".$id_zastepstwa."'> <input type='hidden' name='idPr' value='".$id_pr."'> ";
